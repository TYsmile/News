...
def news_edit_detail():
    ...
    if request.method == "GET":
        ...
        for category in categories:
            # 取到分类的字典
            cate_dict = category.to_dict()
            # 判断当前遍历的分类是否是当前新闻的分类，如果是，则添加 is_selected 为 True
...
def news_edit():
    ...
    content = {"total_page": total_page, "current_page": current_page, "news_list": news_dict_list}

    return render_template('admin/news_edit.html', data=content)
...
def user_count():
    ...
    day_count = 0
    t = time.localtime()
    begin_day_date = datetime.strptime('%d-%02d-01' % (t.tm_year, t.tm_mon), "%Y-%m-%d")
    try:
